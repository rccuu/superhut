...
def load_changelog_section(repo_root: Path, current_tag: str) -> str:
    """Extract the CHANGELOG.md section for the given version tag.

    Looks for a ``## vX.Y.Z`` boundary marker and extracts everything
    between it and the next ``## v...`` marker (or end of file).
    """
    ...
    pattern = re.compile(
        rf"(?ms)^##\s+{re.escape(current_tag)}\s*\n(.*?)(?=^##\s+v[\w.\-]+\s*\n|\Z)"
    )
    match = pattern.search(content)
    if not match:
        return ""
    section = match.group(1).strip()
    # Filter out nested version markers and empty lines-only sections
    lines = [line for line in section.splitlines() if not re.match(r"^##\s+v[\w.\-]+", line)]
    result = "\n".join(lines).strip()
    return result
...
def collect_commits(
    repo_root: Path, previous_tag: str | None, current_tag: str
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--current-tag", required=True, help="Current release tag, e.g. v1.5.5"
    )
    parser.add_argument(
        "--repo", required=True, help="GitHub repository slug, e.g. rccuu/superhut"
    )
...
def main() -> int:
    ...
    notes = render_notes(
        args.repo, args.current_tag, previous_tag, summary, commits, revspec
    )
